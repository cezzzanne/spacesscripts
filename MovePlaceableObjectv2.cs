...
namespace Spaces{
    public class MovePlaceableObjectv2 : MonoBehaviour, IPointerDownHandler, IPointerUpHandler {
    ...
    public void OnPointerDown(PointerEventData eventData){
        ...
        if (isPlacementController) {
            PlacementController.GetComponent<ItemPlacementControllerV2>().SetForwardInput(true, isForward, isReversed);
        } else {
    ...
    public void OnPointerUp(PointerEventData eventData){
        if (isPlacementController) {
            PlacementController.GetComponent<ItemPlacementControllerV2>().SetForwardInput(false, false, false);
        } else {
