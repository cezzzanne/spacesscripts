...
namespace Spaces {
    ...
    public class FriendManagerScriptv2 : MonoBehaviour {
        ...
        // UIManagerPublicScript uiManagerScript;
        ...
        //     if (message.Trim() == "") {
        //         uiManagerScript.ToggleSummonGroupForm(false);
        //         return;
        //     }
        ...
        //             newButton.GetComponent<Button>().onClick.AddListener(()=> {CallOutFriend(member.world_user.id, member.world_user.user.username, member.world_user.world_type);});
        ...
        // public void CallOutFriend(string id, string friendUsername, string worldType) {
        //     DatabaseReference reference = FirebaseDatabase.DefaultInstance.RootReference;
        //     reference.Child("usernameList").Child(friendUsername).GetValueAsync().ContinueWith(task => {
        //         DataSnapshot snapshot = task.Result;
        //         SendNotification(snapshot.Value.ToString().Trim(), "👋 Someone's calling for you!", "@" + username.ToLower() + " is asking you to join them in ~spaces~!");
        //     });
        // }
